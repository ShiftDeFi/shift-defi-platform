...
contract ContainerAgentClaimTest is ContainerAgentBaseTest {
    ...
    function test_ClaimMultiple_1OfN() public {
        uint256 tokensToClaim = 1;
        address[] memory bridgeAdapters = new address[](tokensToClaim);
        bridgeAdapters[0] = address(bridgeAdapter);
        address[] memory tokens = new address[](tokensToClaim);
        tokens[0] = address(dai);
        vm.prank(roles.operator);
        ...
        assertEq(
            uint256(containerAgent.status()),
            uint256(IContainerAgent.ContainerAgentStatus.DepositRequestReceived),
            "test_ClaimMultiple_1OfN: status not DepositRequestReceived"
        );
        assertEq(
            containerAgent.claimCounter(),
            tokenNumber - tokensToClaim,
            "test_ClaimMultiple_1OfN: claim counter not decreased"
        );
        assertEq(
            dai.balanceOf(address(containerAgent)),
            DEPOSIT_AMOUNT,
            "test_ClaimMultiple_1OfN: dai balance not updated"
        );
    }

    function test_ClaimMultiple_NOfN() public {
        uint256 tokensToClaim = tokenNumber;
        address[] memory bridgeAdapters = new address[](tokensToClaim);
        bridgeAdapters[0] = address(bridgeAdapter);
        ...
        assertEq(
            uint256(containerAgent.status()),
            uint256(IContainerAgent.ContainerAgentStatus.BridgeClaimed),
            "test_ClaimMultiple_NOfN: status not BridgeClaimed"
        );
        assertEq(containerAgent.claimCounter(), 0, "test_ClaimMultiple_NOfN: claim counter not 0");
        assertEq(
            dai.balanceOf(address(bridgeAdapter)),
            0,
            "test_ClaimMultiple_NOfN: BridgeAdapter dai balance not zero"
        );
        assertEq(
            notion.balanceOf(address(bridgeAdapter)),
            0,
            "test_ClaimMultiple_NOfN: BridgeAdapter notion balance not zero"
        );
        assertEq(
            dai.balanceOf(address(containerAgent)),
            DEPOSIT_AMOUNT,
            "test_ClaimMultiple_NOfN: ContainerAgent dai balance mismatch"
        );
        assertEq(
            notion.balanceOf(address(containerAgent)),
            DEPOSIT_AMOUNT,
            "test_ClaimMultiple_NOfN: ContainerAgent notion balance mismatch"
        );
    }

    function test_RevertIf_ClaimMultiple_IncorrectContainerStatus() public {
        uint256 tokensToClaim = 1;
        address[] memory bridgeAdapters = new address[](tokensToClaim);
        bridgeAdapters[0] = address(bridgeAdapter);
        address[] memory tokens = new address[](tokensToClaim);
    ...
    function test_RevertIf_ClaimMultiple_ArrayLengthMismatch() public {
        uint256 tokensToClaim = 1;
        address[] memory bridgeAdapters = new address[](tokensToClaim);
        bridgeAdapters[0] = address(bridgeAdapter);
        address[] memory tokens = new address[](tokensToClaim + 1);
        tokens[0] = address(dai);
        tokens[1] = address(notion);
    ...
    function test_RevertIf_ClaimMultiple_InReshufflingMode() public {
        uint256 tokensToClaim = 1;
        address[] memory bridgeAdapters = new address[](tokensToClaim);
        bridgeAdapters[0] = address(bridgeAdapter);
        address[] memory tokens = new address[](tokensToClaim);
    ...
    function test_RevertIf_ClaimMultiple_InEmergencyResolutionMode() public {
        uint256 tokensToClaim = 1;
        address[] memory bridgeAdapters = new address[](tokensToClaim);
        bridgeAdapters[0] = address(bridgeAdapter);
        address[] memory tokens = new address[](tokensToClaim);
